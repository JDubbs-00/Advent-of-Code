...
{
    printf("Number of bags in a shiny gold bag: %d\n", n);
}
...
int bags_inside(Bag *bags, int id)
{
    Bag *b = &bags[id];
    int sum = b->sum;
    int i;
    for (i = 0; i < b->contents_count; i++)
    {
        sum += (b->bag_counts[i] * bags_inside(bags, b->contents[i]));
    }
    return sum;
}
...
int main(int argc, char *argv[])
{
    ...
    while ((length = getline(&line, &len, fp)) != -1)
    {
        ...
        bag->bag_counts = malloc(bag->contents_count * sizeof(int));
        //printf("%d bags can be contained in %s: ", bag->contents_count, bag->name);
        bag->contents_count = 0;
        for (i = 13; line[i] != '\0'; i++)
        {
            if (is_num(line[i]))
            {
                bag->bag_counts[bag->contents_count] = CHAR_TO_NUM(line[i]);
                bag->sum += bag->bag_counts[bag->contents_count];
                i += 2;
                ...
                bag->contents[bag->contents_count++] = j;
                i += 4;
            }
        }
        //printf("\n");
